...
positions <- read.table(file = "/work/hartmaca/regioneR_pred/chrom_pos_all.vcf", sep = " ", col.names = c("CHROM", "POS"))
#create GRanges
gr <- GRanges(seqnames = positions$CHROM, ranges = IRanges(positions$POS, width = 1))

#get job id
task_id <- as.integer(Sys.getenv("SLURM_ARRAY_TASK_ID"))
#names of the 237 predicted miRNAs
names <- read.table(file = "/work/hartmaca/regioneR_pred/regions_predicted_miRNAs.csv", header = TRUE, sep = ",")[,1]
#name for the current job
name <- names[task_id]
pos <- read.table(file = paste0("/data/laub-rna/hiwi/cajus/thesis/data/csv_all_predicted/", name, ".csv"), header = FALSE, sep = ",")
el <- GRanges(seqnames = pos[1,1], ranges = IRanges(start = pos[1,2], end = pos[length(pos[,1]),2]), names = name)
...
write.table(new, file = paste0("/work/hartmaca/regioneR_pred/res_", name, ".csv"), sep = ",", row.names = FALSE, col.names = FALSE, quote = FALSE)
