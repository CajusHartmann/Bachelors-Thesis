...
for(i in 1:length(positions$name)){
  ...
  #read genotype matrix created by VIVA (and filtered by me)
  matrix <- read.table(file = paste("C:/Users/cajus/OneDrive/Dokumente/Bildung/Studium/6. Semester/Bachelorarbeit/Code/VIVA/results_all/filtered/", positions[i,1], "_Genotype_MAF0.005_MR0.5_vcf.csv", sep = ""), header = TRUE, sep = ",")  
  #filter out the positions outside of the miRNA (viva keeps some wrong positions)
  ...
  #I have to check if matrix$pos is empty because originally viva was started with a base of 50bp, but now we filter all invalid positions out
  if(length(matrix$pos) != 0){
  ...
  create_heatmap(t(matrix_all_positions), base_one, miRNA_one, loop, miRNA_two, base_two, 0, 3, name, FALSE, colorscale = list(list(0.0, "white"), list(1/3, "lightblue"), list(2/3, "green"), list(1.0, "red")), path = "C:/Users/cajus/OneDrive/Dokumente/Bildung/Studium/6. Semester/Bachelorarbeit/Code/Create heatmaps/heatmaps_filtered")
}
